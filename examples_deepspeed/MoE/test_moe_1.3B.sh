...
#     DATA_BLEND=/data/the_pile_public_merged_nopreprocessing/pile_text_document
        ...
        DATA_BLEND=/nlp_group/liupeng15/pile
fi
###############################################################################
data_options=" \
...
run_cmd="deepspeed ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} --tokenizer-model $TOKENIZER_PATH &> ${OUTPUT_BASEPATH}/log/${NAME}_${host}_${current_time}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
