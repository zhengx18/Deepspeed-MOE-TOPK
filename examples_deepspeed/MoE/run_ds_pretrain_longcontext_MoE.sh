...
MASTER_ADDR=10.82.120.153
MASTER_PORT=2333
KUBERNETES_CONTAINER_RESOURCE_GPU=8
echo 'DEBUG, RDMA, TOP_K' $DEBUG $RDMA $TOP_K
...
OUTPUT_BASEPATH="/mmu_nlp_hdd/zhengxue/Megatron-DeepSpeed/output1231_tp${TOP_K}"
...
set -x
if [ "${DEBUG}" = "true" ] || [ "${RDMA}" = "false" ]; then
    torchrun --nproc_per_node $KUBERNETES_CONTAINER_RESOURCE_GPU --nnodes $WORLD_SIZE --node_rank $RANK --master_addr $MASTER_ADDR --master_port $MASTER_PORT ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}
else
