...
NNODES=1
GPUS_PER_NODE=8
GPU_NUM=$((${GPUS_PER_NODE}*${NNODES})) # 8
WORLD_SIZE=$((${GPUS_PER_NODE}*${NNODES})) # 8
NODE_RANK=0
MASTER_PORT=6000
...
# --expert-model-parallel-size: Expert Parallelism 的 degree
# NUM_EXPERTS=4
# EXPERT_MODEL_PARALLEL_SIZE=${DP}
...
elif [[ ${MODEL_SIZE} == 1.3 ]]; then HIDDEN_SIZE=2048;  NUM_HEAD=16; NUM_QUERY_GROUP=16; NUM_LAYERS=24; FFN_HIDDEN_SIZE=5488; NORM_EPS=1e-5;
...
# DATA_PATH=/nlp_group/liupeng15/code/querysim/src/corpus_process/export/data.cc.zh
# TOKENIZER_PATH=/nlp_group/liupeng15/toxiansheng/tokenizer.128k.data_ratio
DATA_CACHE_PATH="./data_cache/${LOG_NAME}"
mkdir -p ${DATA_CACHE_PATH}

SAVE_PATH=${BASE_PATH}/checkpoint_experts/${LOG_NAME}
LOG_PATH=${BASE_PATH}/log/${LOG_NAME}/node${NODE_RANK}.log
mkdir -p ${BASE_PATH}/log/${LOG_NAME}
...
DISTRIBUTED_ARGS=" \
       ...
       --sequence-parallel \
       --expert-model-parallel-size ${DP} \
       "    
# --use-distributed-optimizer # 与expert-model-parallel-size 不能同时
